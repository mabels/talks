  ...
  def vagrant_host
    @vagrant_host_cache ||= region.hosts.add("vagrant-host", "flavour" => "ubuntu") do |host|
      host.configip = host.id ||= Construqt::HostId.create do |my|
        my.interfaces << @mac_wlan = region.interfaces.add_bridge(host, "en0",
                                                                  "description" => "en0: Wi-Fi (AirPort)",
                                                                  "mtu" => 1500, "interfaces" => [])
      end
      @mac_usb_ether = region.interfaces.add_bridge(host, "en3",
                                                    "vagrant" => Construqt::Hosts::Vagrant.new.net("bridge").auto_config(false),
                                                    "description" => "en3: USB Ethernet", #ugly vagrant vbox
                                                    "mtu" => 1500, "interfaces" => [])
    end
  end
